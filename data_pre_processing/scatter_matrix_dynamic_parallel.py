...
aoi_gdf = gpd.read_file("../italy_aoi/gadm41_ITA_1.shp")
...
def spatial_fill_nans(raster_path, mps_coord, band_indices):
    with rasterio.open(raster_path) as src:
        ...
        data_xr = xr.DataArray(
            ...
            dims=("points", "time"),
            coords={
                "points": np.arange(data_samples.shape[0]),
                "time": np.arange(data_samples.shape[1]),
            },
        )

        
        data_samples = data_xr.interpolate_na(
            dim="points", method="linear", fill_value="extrapolate"
        )
...
def process_chunk(mps_chunk, i, timestamp, date_band_idx, target_times):
    print(i)
    xs = mps_chunk.iloc[:, 0].values
    ys = mps_chunk.iloc[:, 1].values
    mean_vgd = mps_chunk.iloc[:, 2].values
    ...
    # 1. Seismic Data
    seismic_data = pd.read_csv("../data/dynamic/seismic.csv")
    ...
    seismic_coords = np.array(
        [
            ...
            for lon, lat in seismic_data[["Longitude", "Latitude"]].to_numpy()
        ]
    )
    ...
    with xr.open_mfdataset(
        r"C:\Users\gmfet\Desktop\emilia\data\predictors\dynamic\*.nc", engine="netcdf4"
    ) as ds:
        ds = ds.chunk({"time": -1})
        ds["twsan"] = ds["twsan"].interpolate_na(
...
def main():
    chunk_mps = pd.read_csv(
        "../data_pre_processing/target_static.csv", chunksize=100000
    )
    chunks = list(chunk_mps)
    ...
    with ProcessPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(
                process_chunk, chunk, i, timestamp, date_band_idx, target_times
            )
    ...
    # Concatenate all results and save once
